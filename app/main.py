...
def render_letter(result, nonce):
    ...
    if letter:
        ...
        st.components.v1.html(
            ...
            "<button class='hq-tool-btn' id='hqCopyBtn' onclick='hqCopy()'>⧉ Copy letter</button>"
            "<button class='hq-tool-btn' id='hqSpeakBtn' onclick='hqSpeak()'>▶ Read aloud</button>"
            "<button class='hq-tool-btn' onclick='hqStop()'>■ Stop</button>"
            "</div>"
            "<script>var HQ_LETTER=" + payload + ";var HQ_LANG='" + speak_code + "';"
            ...
            "function hqCopy(){var b=document.getElementById('hqCopyBtn');var ok=false;"
            "try{var t=document.createElement('textarea');t.value=HQ_LETTER;t.setAttribute('readonly','');"
            "t.style.position='fixed';t.style.left='-1000px';t.style.top='0';t.style.opacity='0';"
            "document.body.appendChild(t);t.focus();t.select();t.setSelectionRange(0,t.value.length);"
            "ok=document.execCommand('copy');document.body.removeChild(t);}catch(e){ok=false;}"
            "if(ok){b.innerText='✓ Copied';setTimeout(function(){b.innerText='⧉ Copy letter';},1600);return;}"
            "if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(HQ_LETTER).then(function(){b.innerText='✓ Copied';setTimeout(function(){b.innerText='⧉ Copy letter';},1600);}).catch(function(){b.innerText='Press Ctrl+C to copy';});return;}"
            "b.innerText='Press Ctrl+C to copy';}"
            "function hqSpeak(){if(!('speechSynthesis' in window)){return;}var sy=window.speechSynthesis;sy.cancel();"
            "var u=new SpeechSynthesisUtterance(HQ_LETTER);u.lang=HQ_LANG;"
            "var vs=sy.getVoices()||[];var base=HQ_LANG.toLowerCase().split('-')[0];"
            "for(var i=0;i<vs.length;i++){if(vs[i].lang&&vs[i].lang.toLowerCase().indexOf(base)===0){u.voice=vs[i];break;}}"
            "sy.speak(u);}"
            "function hqStop(){if('speechSynthesis' in window){window.speechSynthesis.cancel();}}"
            "</script>"
